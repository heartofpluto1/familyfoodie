...
// Dynamic import with ssr: false - component only loads on client
const ShoppingListWithDnd = dynamic(() => import('./components/ShoppingListWithDnd'), {
	ssr: false,
});
...
function ShoppingListContent({ weekDateRange }: { weekDateRange: string }) {
	...
	// Single render - content will be wrapped with DndContext via dynamic import
	const content = (
	...
	// Wrap content with DndContext via dynamic import (client-only)
	return (
		<ShoppingListWithDnd
