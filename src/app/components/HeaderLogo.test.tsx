...
describe('HeaderLogo Component', () => {
	...
	describe('Mobile Menu Interactions', () => {
		...
		it('shows admin link in mobile menu for admin users', () => {
			...
			const mobileAdminLink = mobileAdminLinks.find(link => link.closest('nav')?.classList.contains('sm:hidden'));
		...
		it('maintains proper mobile menu styling', () => {
			...
			expect(summary).toHaveClass('list-none', 'cursor-pointer', 'bg-surface', 'border', 'border-custom', 'rounded-sm');
		});
