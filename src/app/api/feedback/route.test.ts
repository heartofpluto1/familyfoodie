...
import type { FeedbackSubmission, Feedback, FeedbackStats } from '@/types/feedback';
	...
	describe('GET /api/feedback', () => {
		...
		describe('Success Path Tests', () => {
			...
			it('returns feedback with stats when includeStats=true', async () => {
				await testApiHandler({
					appHandler,
					url: '/api/feedback?includeStats=true',
					test: async ({ fetch }) => {
						const response = await fetch({
							method: 'GET',
						});
			...
			it('handles query parameters correctly', async () => {
				await testApiHandler({
					appHandler,
					url: '/api/feedback?status=new&category=bug&rating=5&limit=10&offset=5',
					test: async ({ fetch }) => {
						const response = await fetch({
							method: 'GET',
						});
			...
			it('handles date range filters', async () => {
				await testApiHandler({
					appHandler,
					url: '/api/feedback?startDate=2024-01-01&endDate=2024-12-31',
					test: async ({ fetch }) => {
						const response = await fetch({
							method: 'GET',
						});
			...
			it('validates and handles invalid numeric parameters', async () => {
				await testApiHandler({
					appHandler,
					url: '/api/feedback?rating=invalid&limit=abc&offset=xyz',
					test: async ({ fetch }) => {
						const response = await fetch({
							method: 'GET',
						});
