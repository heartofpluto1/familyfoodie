			...
			<div className="min-h-screen bg-background flex items-center justify-center px-4">
				<div className="max-w-md w-full">
					<div className="bg-surface border border-custom rounded-sm p-8 text-center">
						...
						<button onClick={() => router.push('/')} className="px-4 py-2 bg-accent text-background rounded-sm hover:bg-accent/90 transition-colors">
							Go to Home
						</button>
					</div>
				</div>
			...
			<div className="max-w-md w-full">
				<div className="bg-surface border border-custom rounded-sm p-8">
					...
					<p className="text-xs text-secondary text-center mt-4">
						By accepting this invitation, you&apos;ll join the {invitation.householdName} household for shared meal planning on Family Foodie.
					</p>

					{/* Expiration Notice */}
					<p className="text-xs text-muted text-center mt-4">This invitation expires on {new Date(invitation.expiresAt).toLocaleDateString()}</p>
				</div>
			</div>
		</div>
	);
