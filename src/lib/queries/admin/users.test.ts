...
describe('Admin Users Queries', () => {
	...
	describe('getAllUsers', () => {
		it('returns all users ordered by date_joined DESC', async () => {
			mockPool.execute.mockResolvedValue([mockUsers as RowDataPacket[], []]);

			const result = await getAllUsers();

			expect(result).toEqual(mockUsers);
			expect(mockPool.execute).toHaveBeenCalledWith(expect.stringContaining('SELECT'));
			expect(mockPool.execute).toHaveBeenCalledWith(expect.stringContaining('ORDER BY u.date_joined DESC'));
		});

		it('returns empty array when no users exist', async () => {
			mockPool.execute.mockResolvedValue([[] as RowDataPacket[], []]);
	...
	describe('getUserById', () => {
		it('returns user when found', async () => {
			mockPool.execute.mockResolvedValue([[mockUsers[0]] as RowDataPacket[], []]);

			const result = await getUserById(1);

			expect(result).toEqual(mockUsers[0]);
			expect(mockPool.execute).toHaveBeenCalledWith(expect.stringContaining('WHERE u.id = ?'), [1]);
		});
		...
		it('uses parameterized query to prevent SQL injection', async () => {
			...
			expect(mockPool.execute).toHaveBeenCalledWith(expect.any(String), [1]);
		});
	});
	...
	describe('updateUser', () => {
		...
		it('returns false when user not found', async () => {
			mockPool.execute.mockResolvedValue([{ affectedRows: 0 } as ResultSetHeader, []]);
	...
	describe('deleteUser', () => {
		it('deletes user successfully', async () => {
			...
			expect(result).toBe(true);
			expect(mockPool.execute).toHaveBeenCalledWith('DELETE FROM users WHERE id = ?', [1]);
		});
	...
	describe('getUserStats', () => {
		...
		it('returns user statistics', async () => {
			mockPool.execute.mockResolvedValue([mockStatsResult as RowDataPacket[], []]);
			...
			expect(mockPool.execute).toHaveBeenCalledWith(expect.stringContaining('COUNT(*) as total'));
		});
