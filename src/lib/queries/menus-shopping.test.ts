  ...
  describe('resetShoppingListFromRecipesHousehold', () => {
    it('should reset shopping list for specific household', async () => {
      ...
      expect(mockConnection.execute).toHaveBeenCalledWith(
        'DELETE FROM shopping_lists WHERE week = ? AND year = ? AND household_id = ?',
        [32, 2024, 1]
      );
      ...
      expect(mockConnection.execute).toHaveBeenCalledWith(
        expect.stringContaining('INSERT INTO shopping_lists (week, year, household_id, fresh, name, sort, cost, recipeIngredient_id, purchased, stockcode)'),
        expect.any(Array)
      );
    });
    ...
    it('should group ingredients correctly by ingredient_id and quantityMeasure_id', async () => {
      ...
      const insertCall = mockConnection.execute.mock.calls.find(call =>
        call[0].includes('INSERT INTO shopping_lists')
      );
    ...
    it('should order ingredients by category for consistent shopping list', async () => {
      ...
      expect(mockConnection.execute).toHaveBeenCalledWith(
        expect.stringContaining('WHEN i.fresh = 1 THEN i.supermarketCategory_id'),
        [32, 2024, 1]
      );
    });
