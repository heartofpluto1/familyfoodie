 ...
 */
export async function updateJunctionTableForCollectionRecipe(connection: PoolConnection, collectionId: number, oldRecipeId: number, newRecipeId: number): Promise<void> {
  await connection.execute(`
 ...
 */
export async function updateRecipeIngredientsForHousehold(connection: PoolConnection, oldIngredientId: number, newIngredientId: number, householdId: number): Promise<void> {
  await connection.execute(`
...
export async function deleteOrphanedIngredients(connection: PoolConnection, householdId: number, excludeRecipeId?: number): Promise<number[]> {
  ...
  const orphanedIds = (rows as any[]).map(row => row.id);
