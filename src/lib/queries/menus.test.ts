...
describe('Household-Aware Recipe Queries', () => {
	...
	describe('getAllRecipesWithDetailsHousehold', () => {
		it('should return recipes with household precedence and details', async () => {
			...
			expect(mockPool.execute).toHaveBeenCalledWith(expect.stringContaining('ORDER BY r.name ASC'), [1, 1, 1]);
		});
		...
		it('should filter by collection when provided', async () => {
			...
			expect(mockPool.execute).toHaveBeenCalledWith(expect.stringContaining('AND cr.collection_id = ?'), [1, 1, 1, 10]);
		});
		...
		it('should include public and subscribed collections', async () => {
			...
			expect(mockPool.execute).toHaveBeenCalledWith(expect.stringContaining('c.public = 1 AND cs.household_id IS NOT NULL'), [1, 1, 1]);
		});
	});
